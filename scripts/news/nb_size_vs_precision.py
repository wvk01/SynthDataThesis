...
for size in sample_sizes:
    ...
    # Train Naïve Bayes classifier on synthetic augmented data
    nb_classifier_synthetic = MultinomialNB()
    nb_classifier_synthetic.fit(X_train_tfidf_synthetic, y_train_synthetic)

    # Train Naïve Bayes classifier on real-world augmented data
    ...
    test_precisions_synthetic = [evaluate_test_set(test_set_path, tfidf_vectorizer_synthetic, nb_classifier_synthetic) for test_set_path in test_set_paths]
    ...
    test_precisions_real_world = [evaluate_test_set(test_set_path, tfidf_vectorizer_real_world, nb_classifier_real_world) for test_set_path in test_set_paths]
...
plt.title('Sample Size vs Average Precision (Naive Bayes)')
plt.grid(True)
plt.legend()
plt.show()
