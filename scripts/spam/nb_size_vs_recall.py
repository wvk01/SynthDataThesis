...
def evaluate_test_set(test_data_path, tfidf_vectorizer, nb_classifier):
    ...
    _, recall, _, _ = precision_recall_fscore_support(y_test, y_pred, average=None, labels=[1])  # Label '1' is spam email
    return recall[0]
...
for size in sample_sizes:
    ...
    # Train Naive Bayes classifier on synthetic augmented data
    nb_classifier_synthetic = MultinomialNB()
    nb_classifier_synthetic.fit(X_train_tfidf_synthetic, y_train_synthetic)

    # Train Naive Bayes classifier on real-world augmented data
    nb_classifier_real_world = MultinomialNB()
    nb_classifier_real_world.fit(X_train_tfidf_real_world, y_train_real_world)

    # Evaluate on all test sets and calculate average recall for synthetic augmented data
    test_recalls_synthetic = [evaluate_test_set(test_set_path, tfidf_vectorizer_synthetic, nb_classifier_synthetic) for test_set_path in test_set_paths]
    ...
    test_recalls_real_world = [evaluate_test_set(test_set_path, tfidf_vectorizer_real_world, nb_classifier_real_world) for test_set_path in test_set_paths]
...
plt.title('Sample Size vs Average Recall (Naive Bayes)')
plt.grid(True)
plt.legend()
plt.show()
