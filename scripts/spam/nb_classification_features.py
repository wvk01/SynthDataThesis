...
train_data = pd.read_csv('../../data/spam_data/preprocessed/real_world/processed_additional_train_data.csv')
...
# Extract feature names and feature log probabilities
feature_names = tfidf_vectorizer.get_feature_names_out()
feature_log_prob = nb_classifier.feature_log_prob_[1]  # Log-probabilities for the spam class (label '1')

# Pair feature names with their importance (log-probabilities)
feature_importances = sorted(zip(feature_log_prob, feature_names), reverse=True)

# Print the top 20 most important features for predicting spam
print("Top 20 Features Contributing to Spam Classification:")
for log_prob, feature in feature_importances[:20]:
    print(f"{feature}: {np.exp(log_prob):.4f}")

# Continue with your evaluation and averaging code
accuracies = []
precisions = []
recalls = []
f1_scores = []
